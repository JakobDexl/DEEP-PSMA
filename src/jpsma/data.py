...
def get_combined_affine_matrix_3d(angles_deg=(0.0, 0.0, 0.0), scales=1.0, device="cpu"):
    """
...
def apply_small_rotation_gpu(vol_dict, angles, scale):
    ...
    for k, vol in vol_dict.items():
        ...
        grid = F.affine_grid(affine[:3].unsqueeze(0), size=[1, 1, D, H, W], align_corners=True)  # [1,3,4]
...
def random_flip_3d(volume_dict):
    ...
    if random.random() < 0.5:
        axes.append(0)  # flip Z
    if random.random() < 0.5:
        axes.append(1)  # flip Y
    if random.random() < 0.5:
        axes.append(2)  # flip X
...
class PETValDataset(Dataset):
    ...
    def load_modality(self, case_path, modality):
        ...
        return {"PET": pet, "TTB": ttb * (pet >= 0), "TOTSEG": totseg, "CT": ct, "SUV_T": suv_threshold, "meta": meta}
...
class PETTrainDataset(Dataset):
    ...
    def load_modality(self, case_path, modality):
        ...
        return {"PET": pet, "TTB": ttb, "TOTSEG": totseg, "CT": ct, "SUV_T": suv_threshold, "meta": meta}
    ...
    def crop_volume(self, vol, coords, pad_value=0):
        ...
        pad_width = ((pad_before_z, pad_after_z), (pad_before_y, pad_after_y), (pad_before_x, pad_after_x))
    ...
    def __getitem__(self, idx):
        ...
        def random_rotation(vols, max_angle=10):
            ...
            return apply_small_rotation_gpu(vols, angles, (1.0, 1.0, 1.0))

        def random_scale(vols, ran=(0.9, 1.3)):
            scale = [random.uniform(ran[0], ran[1]) for _ in range(3)]
            return apply_small_rotation_gpu(vols, (0.0, 0.0, 0.0), scale)
        ...
        ops.append(lambda: random_scale(vols, ran=(0.9, 1.4)))
...
class PETDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        if self.folds_file is not None:
            ...
            base_case_ids = sorted(set(re.sub(r"_(to_fdg|to_psma)$", "", cid) for cid in self.case_ids))
        ...
        self.val_dataset = PETValDataset(
            self.root_dir, self.val_ids, min_size=self.crop_size, normalize=self.normalize, load_ct=self.use_ct
        )
