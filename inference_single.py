...
def interf0_handler():
    ...
    # models = [NNUnet.load_from_checkpoint("weights/" + p, map_location="cuda", sw_batch_size=1).eval().cuda() for p in model_paths]
    model = NNUnet.load_from_checkpoint("weights/f1_loss=0.0800.ckpt", map_location="cuda", sw_batch_size=1)
    model.eval().cuda()
    print("X")
    tmp = {}
    for i, tracer in enumerate(["FDG", "PSMA"]):
        ...
        with torch.no_grad():
            with torch.amp.autocast("cuda", dtype=torch.float32):
                ...
                preds = torch.ge(torch.sigmoid(logit), 0.5)  # multi-label output in [0,1]
    ...
    fdg_mask = tmp["FDG"]["FDG"]
    psma_mask = tmp["PSMA"]["PSMA"]
